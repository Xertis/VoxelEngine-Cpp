...
bytes:append(value: number)
bytes:append(values: Bytearray | table<number>)

-- Вставляет данные по индексу.
bytes:insert(index: number, value: number)

-- Удаляет элемент(ы), если передан `count`, удалит `count` кол-во элементов с выбранного индекса.
bytes:remove(index: number, [опционально] count: number)

-- Очищает массив
bytes:clear()

-- Создаёт новый Bytearray, содержащий копию части данных с offset до offset+length
bytes:slice(offset: number, length: number) -> Bytearray
```
