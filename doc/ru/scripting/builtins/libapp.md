...
-- Выгружает загруженный контент, очищая загруженные модули, окружение паков и объявленные ивенты
-- * packs: содержит массив id паков, окружение которых не будет очищено после ресета контента
app.reset_content([опционально] packs: table<string>)
